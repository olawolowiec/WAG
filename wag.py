...
### ERRORS ###
class Error:
    def __init__(self, pos_start, pos_end, error_name):
...
class IllegalCharError(Error):
    def __init(self, pos_start, pos_end):
...
class Lexer:
    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            if self.current_char in ' \t':
                ...
                tokens.append(Token(plusXD, pos_start=self.pos))
                self.advance()
            elif self.current_char == '-':
                tokens.append(Token(minusXD, pos_start=self.pos))
                self.advance()
            elif self.current_char == '*':
                ...
                tokens.append(Token(lparenXD, pos_start=self.pos))
                self.advance()
            elif self.current_char == ')':
                tokens.append(Token(rparenXD, pos_start=self.pos))
                self.advance()
            else:
        ...
        tokens.append(Token(eofXD, pos_start=self.pos))
        return tokens, None
    ...
    def make_number(self):
        ...
        while self.current_char != None and self.current_char in digits + '.':
            ...
            self.advance()
        if dot_count == 0:
            return Token(intXD, int(num_str), pos_start, self.pos)
        else:
            return Token(floatXD, float(num_str), pos_start, self.pos)
